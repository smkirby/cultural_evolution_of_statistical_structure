...
data["sentence_number"] = list(range(60))*66
data
```
...
model <- lmer(data=py$data, error ~ generation + (1+generation|chain) + (1|sentence_number))
summary(model)
```
...
def transition_data(data, generation, chain, n):
    ...
    for string in strings:
        ...
        for i in range(len(string) - (n - 1)):
            transitions.append(ngrams[string[i:i+n]] / smaller_grams[string[i:i+(n-1)]])
